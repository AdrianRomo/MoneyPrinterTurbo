---
name: influencer-automation-video
description: Use this skill whenever the user wants to create a finished video from a topic, title, idea, prompt, or script with Influencer-Automation 2.0. This includes short-form, voice-over, educational, marketing, social-media, and stock-footage videos. Also use it when the user mentions Influencer-Automation 2.0, provides this Skill URL, asks an AI agent to install or configure Influencer-Automation 2.0, needs missing API keys identified, wants a failed generation repaired, or wants a generated MP4 located and delivered. Use this skill when the expected outcome is a final video file, not setup instructions.
...
metadata:
  ...
  version: "1.3.2"
  upstream: "https://github.com/AdrianRomo/Influencer-Automation-2.0"
---

# Influencer-Automation 2.0 Video Generation
...
Unless the user requests otherwise, generate one Chinese `9:16` portrait video with Pexels footage, the default Chinese Edge TTS voice, subtitles, and background music. Install Influencer-Automation 2.0 under the user's home directory.

## Execution

### 1. Locate the helper

Resolve `SKILL_DIR` from this `SKILL.md` file. The helper is the adjacent `influencer_automation_agent.py`. Set the terminal tool's working directory to `SKILL_DIR` and invoke the helper by its relative filename. Do not put the absolute helper path in the command, and do not run an extra `ls` or `dir` check.

This is required on Windows because some agent terminal validators remove backslashes from absolute paths embedded in commands. Using `influencer_automation_agent.py` with `workdir=SKILL_DIR` avoids that failure and works on both macOS and Windows.
...
```text
https://raw.githubusercontent.com/AdrianRomo/Influencer-Automation-2.0/main/docs/skill/influencer_automation_agent.py
```
...
```bash
uv run --no-project --python 3.11 python influencer_automation_agent.py --subject "<video topic>"
```
...
`influencer_automation_agent.py` emits `VIDEO_FILE` only after confirming that the file exists and is non-empty. Do not run another `ls`, `stat`, or file validation command.

If the terminal reports `exitCode=0` but truncates the output or returns a history-file reference without `IA2_RESULT`, do not infer failure and do not inspect old logs. Read this file once:

```text
~/influencer-automation-2.0/.agent-logs/influencer-automation-video/latest-result.json
```
...
`IA2_NEEDS_INPUT` includes only the required fields, recommended LLM providers and signup links, custom OpenAI-compatible requirements, and the Pexels signup link. Ask only for the listed values and do not request credentials already found in `config.toml`.
...
Use `IA2_ERROR` and `LOG_FILE` to repair a recoverable problem and retry once. Ask the user only if the repair requires a new API key. If the retry fails, report the failed stage, a short error, and the log path.

A terminal-tool path validation error is not a video-generation failure because the helper did not start. Correct the working directory and retry the relative command once. Never ask the user to copy `influencer_automation_agent.py`, run commands manually, or confirm whether the agent should continue.
...
- Use uv and the Influencer-Automation 2.0 CLI only.
